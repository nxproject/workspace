    ...
    statics: {
      ...
      ready: function ready(callback) {
        ...
        var onWindowLoad = function onWindowLoad() {
          qx.module.Event.__isReady__P_141_2 = true;
          callback();
        };
        ...
        if (qxWeb.env.get("engine.name") !== "mshtml" || qxWeb.env.get("browser.documentmode") > 8) {
          ...
          var timer = function timer() {
            ...
            if (qx.module.Event.__isReady__P_141_2) {
              return;
            }
      ...
      $registerEventNormalization: function $registerEventNormalization(types, normalizer) {
        ...
        var registry = qx.module.Event.__normalizations__P_141_0;
      ...
      $getEventNormalizationRegistry: function $getEventNormalizationRegistry() {
        return qx.module.Event.__normalizations__P_141_0;
      },
      ...
      $registerEventHook: function $registerEventHook(types, registerHook, unregisterHook) {
        ...
        var onHooks = qx.module.Event.__hooks__P_141_1.on;
        ...
        var offHooks = qx.module.Event.__hooks__P_141_1.off;
      ...
      $getEventHookRegistry: function $getEventHookRegistry() {
        return qx.module.Event.__hooks__P_141_1;
      }
    },
    members: {
      /**
      ...
      on: function on(type, listener, context, useCapture) {
        for (var i = 0; i < this.length; i++) {
          ...
          var hooks = qx.module.Event.__hooks__P_141_1.on; // generic
          ...
          var bound = function (el, event) {
            ...
            var registry = qx.module.Event.__normalizations__P_141_0; // generic
          ...
          if (!el.__listener__P_141_3) {
            el.__listener__P_141_3 = {};
          }

          if (!el.__listener__P_141_3[type]) {
            el.__listener__P_141_3[type] = {};
          }

          el.__listener__P_141_3[type][el.$$lastlistenerId] = bound;

          if (!context) {
            // store a reference to the dynamically created context so we know
            // what to check for when removing the listener
            if (!el.__ctx__P_141_4) {
              el.__ctx__P_141_4 = {};
            }

            el.__ctx__P_141_4[el.$$lastlistenerId] = ctx;
          }
        }
      ...
      off: function off(type, listener, context, useCapture) {
        ...
        for (var j = 0; j < this.length; j++) {
          ...
          if (type !== null) {
            ...
            for (var listenerType in el.__listener__P_141_3) {
              types.push(listenerType);
            }
          }

          for (var i = 0, l = types.length; i < l; i++) {
            for (var id in el.__listener__P_141_3[types[i]]) {
              var storedListener = el.__listener__P_141_3[types[i]][id];

              if (removeAll || storedListener == listener || storedListener.original == listener) {
                // get the stored context
                var hasStoredContext = typeof el.__ctx__P_141_4 !== "undefined" && el.__ctx__P_141_4[id];
                var storedContext;

                if (!context && hasStoredContext) {
                  storedContext = el.__ctx__P_141_4[id];
                ...
                if (result !== null) {
                  delete el.__listener__P_141_3[types[i]][id];
                }

                if (hasStoredContext) {
                  delete el.__ctx__P_141_4[id];
                }
              }
            } // call hooks


            var hooks = qx.module.Event.__hooks__P_141_1.off; // generic
...
//# sourceMappingURL=Event.js.map?dt=1598045178289