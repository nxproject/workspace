...
qx.Class.define("qxl.upload.FormHandler", {
  ...
  members: {
    ...
    _createFile: function(input) {
      var id = "upload-" + this._getUniqueFileId(), filename = input.value.replace(/.*(\/|\\)/, ""), file = new qxl.upload.File(
          input, filename, id);
      return file;
    },
    ...
    _doCancel: function(file) {
      var data = file.getUserData("qxl.upload.FormHandler");
      if (!data)
        return;
