 ...
 * 
 * Call qxl.upload.XhrHandler.isSupported() to check whether this
 * class can be used (otherwise use FormHandler)
 */
qx.Class.define("qxl.upload.XhrHandler", {
  extend: qxl.upload.AbstractHandler,
  ...
  members: {
    ...
    addBlob: function (filename, blob, params){
     ...
     var file = new qxl.upload.File(blob, filename, id);
     if (params) {
    ...
    _createFile: function(input) {
      ...
      for (var i = 0; i < bomFiles.length; i++) {
        ...
        var file = new qxl.upload.File(bomFile, filename, id);
    ...
    _doUpload: function(file) {
      ...
      if (qxl.upload.XhrHandler.isWithCredentials())
        xhr.withCredentials = true;

      var self = this;

      file.setUserData("qxl.upload.XhrHandler", xhr);
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState == 4) {
          ...
          // response);
          file.setUserData("qxl.upload.XhrHandler", null);
    ...
    _doCancel: function(file) {
      var xhr = file.getUserData("qxl.upload.XhrHandler");
      if (xhr) {
        xhr.abort();
        file.setUserData("qxl.upload.XhrHandler", null);
      }
